...
async function syncShuttleRequestToSupabase(input: {
  ...
  if (!isSupabaseConfigured || !supabase) {
    console.log("Sunday Shuttle: Supabase is not configured. Local save only.");
    return;
  }

  const { error } = await supabase.from("shuttle_requests").insert({
  ...
  if (error) {
    console.warn("Sunday Shuttle: Supabase insert failed:", error.message);
  }
}
...
export async function createDriverListing(input: {
  ...
  await syncShuttleRequestToSupabase({
